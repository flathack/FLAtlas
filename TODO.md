...
- [ ] Naechster sinnvoller Arbeitsblock: `flight_mode.py` nach den ausgelagerten Navigationshilfen weiter auf HUD-/State-/Orbit-Helfer pruefen, parallel `view_3d.py` als verbleibende Render-Orchestrierung final bewerten und danach `dialogs.py` als UI-Orchestrierung sauber einordnen.
...
- Naechste Arbeitsbloecke:
  - `flight_mode.py` nach den Navigations-Refactors auf weitere abtrennbare HUD-/State-/Orbit-Helfer pruefen.
...
- Naechste Arbeitsbloecke:
  - `flight_mode.py` Rest nach den Navigations-Refactors in klar abgegrenzte Pure-Logic-/State-Helfer zerlegen.
