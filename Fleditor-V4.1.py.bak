...
from PySide6.QtCore import Qt, QPointF, QRectF, Signal, QUrl, QObject, QEvent
from PySide6.QtGui import QBrush, QColor, QPen, QFont, QPainter, QAction, QTransform, QPolygonF, QKeySequence, QShortcut, QVector3D, QQuaternion
...
class System3DView(QWidget):
    ...
    def __init__(self):
        super().__init__()
        self._obj_map: dict[object, tuple[object, object]] = {}
        self._zone_map: dict[object, tuple[object, object]] = {}
        self._zone_entities: list[object] = []
        self._obj_component_refs: dict[object, list[object]] = {}
        self._zone_component_refs: dict[object, list[object]] = {}
        # Per-object: sphere wrapper entity + cube wrapper entity (pre-created, toggled via setParent)
        self._obj_sphere_ent: dict[object, object] = {}   # obj -> sphere child entity
        self._obj_cube_ent: dict[object, object] = {}     # obj -> cube child entity
        self._obj_cube_refs: dict[object, list[object]] = {}  # prevent GC
        self._selected_obj = None
        self._axis_gizmo_entities: list[object] = []
        self._axis_gizmo_refs: list[object] = []
        self._axis_gizmo_mats: dict[str, object] = {}  # axis_name -> material (for glow)
        self._axis_step_world = 120.0
        self._move_mode = False  # controlled by main window move checkbox
        self._drag_mode = None
        self._locked_axis = None   # 'x','y','z' when an axis arrow is clicked and locked
    ...
    def _build_ui(self):
        ...
        self._controls_hint = QLabel(
            "3D: Linke Maus = Orbit · Rechte Maus = Pan · Mausrad = Zoom · Move-Modus: Pfeil klicken → Mausrad = Verschieben"
        )
        self._controls_hint.setWordWrap(False)
        self._controls_hint.setFixedHeight(22)
        self._controls_hint.setStyleSheet(
            "QLabel { background: rgba(0, 0, 0, 120); color: #E6E6E6; padding: 1px 8px; font-size: 11px; }"
        )
    ...
    def center_on_item(self, item):
        if item in self._obj_map:
            _ent, tr = self._obj_map[item]
            self._cam_target = tr.translation()
            self._cam_pitch = 1.42
            self._cam_yaw = 0.0
            self._cam_distance = max(120.0, self._system_radius * 0.45)
            self._update_camera()
            return
        if item in self._zone_map:
            _ent, tr = self._zone_map[item]
            self._cam_target = tr.translation()
            self._cam_pitch = 1.42
            self._cam_yaw = 0.0
            self._cam_distance = max(180.0, self._system_radius * 0.6)
            self._update_camera()

    def eventFilter(self, obj, event):
        # Global wheel interception when a gizmo axis is locked
        if event.type() == QEvent.Wheel and self._locked_axis and self._selected_obj:
            ...
            adx = step if ax == "x" else 0.0
            ady = step if ax == "y" else 0.0
            adz = step if ax == "z" else 0.0
            self.object_axis_delta.emit(self._selected_obj, adx, ady, adz)
            return True
        ...
        if et == QEvent.MouseButtonPress:
            ...
            if event.button() == Qt.RightButton:
                self._drag_mode = "pan"
                return True
        elif et == QEvent.MouseMove and self._last_mouse_pos is not None and self._drag_mode:
            pos = event.position()
            ...
            delta = event.angleDelta().y()
            # If an axis is locked, scroll wheel moves along that axis
    ...
    def _clear_all_obj_entities(self):
        """Remove all object entities from scene (called by clear_scene)."""
        for ent, _tr in self._obj_map.values():
            ent.setParent(None)
        self._obj_map.clear()
        self._obj_component_refs.clear()
    ...
    def set_move_mode(self, enabled: bool):
        """Called by main window when move checkbox is toggled."""
    ...
    def _on_axis_gizmo_clicked(self, axis: str):
        ...
        if self._locked_axis == axis:
            # Unlock: click same arrow again → deactivate
            self._locked_axis = None
            self._reset_gizmo_colors()
            if app:
                app.removeEventFilter(self)
        else:
            # Lock this axis: highlight it
            self._locked_axis = axis
            self._highlight_gizmo_axis(axis)
            if app:
                app.installEventFilter(self)
        # Ensure container has focus so it receives subsequent wheel events
        container = getattr(self, '_container', None)
        if container is not None:
            container.setFocus(Qt.OtherFocusReason)

    def _highlight_gizmo_axis(self, axis: str):
        """Make the locked axis arrow glow bright, dim the others."""
    ...
    def _reset_gizmo_colors(self):
        """Reset all gizmo arrows to their default colors."""
    ...
    def _show_axis_gizmo(self, center: QVector3D):
        ...
        configs = [
            ("x", QColor(255, 80, 80), QVector3D(20.0, 0.0, 0.0), QQuaternion.fromAxisAndAngle(0.0, 0.0, 1.0, -90.0)),
            ("y", QColor(80, 220, 80), QVector3D(0.0, 20.0, 0.0), QQuaternion()),
            ("z", QColor(80, 140, 255), QVector3D(0.0, 0.0, 20.0), QQuaternion.fromAxisAndAngle(1.0, 0.0, 0.0, -90.0)),
        ]
        ...
        for axis_name, color, offset, rotation in configs:
            ...
            arrow_pick.clicked.connect(lambda *_a, ax=axis_name: self._on_axis_gizmo_clicked(ax))
    ...
    def set_data(self, objects, zones, scale: float):
        ...
        if self._obj_map:
            ...
            radius = max(span_x, span_z, span_y * 0.5, 120.0)
        ...
        # Top-down start so switch from 2D feels natural.
        self._cam_yaw = 0.0
        self._cam_pitch = 1.42
        self._update_camera()
    ...
    def _create_object_entity(self, obj, scale: float):
        arch = obj.data.get("archetype", "").lower()
        name = obj.nickname.lower()
        is_trade_lane_ring = any(tag in name or tag in arch for tag in ("trade_lane_ring", "tradelane_ring"))

        ent = QEntity3D(self._root)
        tr = QTransform3D()
        mesh = None
        sun_radius = 0.0
        if is_trade_lane_ring:
            mesh = QSphereMesh3D()
            mesh.setRadius(1.2)
            ...
            # Default: sphere for all objects (performance-friendly)
            mesh = QSphereMesh3D()
            mesh.setRadius(2.8)

        mat = QPhongMaterial3D(self._root)
        mat.setDiffuse(self._obj_color(obj))
        try:
            if sun_radius > 0:
                mat.setAmbient(QColor(255, 245, 170))
            else:
                mat.setAmbient(self._obj_color(obj).lighter(175))
        except Exception:
            pass
        ...
        picker.clicked.connect(lambda *_args, o=obj: self.object_selected.emit(o))

        # -- Sphere child entity (enabled by default) --
        sphere_ent = QEntity3D(ent)
        sphere_ent.addComponent(mesh)
        sphere_ent.addComponent(mat)
        self._obj_sphere_ent[obj] = sphere_ent

        # -- Cube child entity (disabled by default, enabled on selection) --
        cube_wrapper = QEntity3D(ent)  # always in scene graph
        ...
        self._obj_cube_refs[obj] = cube_refs  # prevent GC

        component_refs = [tr, picker, mesh, mat, sphere_ent, cube_wrapper]
        ent.addComponent(tr)
    ...
    def _create_orientation_cube(self, parent_ent, obj, cube_size: float):
        ...
        face_specs = [
            (QColor(255, 40, 40), QVector3D(0.0, 0.0, half), QVector3D(cube_size, cube_size, thick)),
            (QColor(80, 220, 220), QVector3D(0.0, 0.0, -half), QVector3D(cube_size, cube_size, thick)),
            (QColor(80, 220, 80), QVector3D(half, 0.0, 0.0), QVector3D(thick, cube_size, cube_size)),
            (QColor(70, 130, 255), QVector3D(-half, 0.0, 0.0), QVector3D(thick, cube_size, cube_size)),
            (QColor(245, 235, 80), QVector3D(0.0, half, 0.0), QVector3D(cube_size, thick, cube_size)),
            (QColor(220, 80, 220), QVector3D(0.0, -half, 0.0), QVector3D(cube_size, thick, cube_size)),
        ]
        ...
        for color, trans, ext in face_specs:
            ...
            face_picker.clicked.connect(lambda *_args, o=obj: self.object_selected.emit(o))
    ...
    def _show_cube_hide_sphere(self, obj):
        """Show orientation cube, hide sphere for this object."""
    ...
    def _show_sphere_hide_cube(self, obj):
        """Show sphere, hide orientation cube for this object."""
        cube_ent = self._obj_cube_ent.get(obj)
        sphere_ent = self._obj_sphere_ent.get(obj)
        if cube_ent is not None:
    ...
    def set_selected(self, obj):
        ...
        # If re-selecting the same object, don't re-swap (prevents gizmo flicker)
        if new_obj is not None and new_obj is self._selected_obj:
            return
        # Revert previous selection: cube → sphere
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.move_cb.toggled.connect(lambda checked: self.view3d.set_move_mode(checked) if hasattr(self, 'view3d') else None)
        ...
        self.view3d_switch = QCheckBox("3D Ansicht")
        self.view3d_switch.setToolTip("Zwischen 2D- und 3D-Systemansicht wechseln")
    ...
    def _toggle_3d_view(self, enabled: bool):
        # In der Universumsansicht (kein geöffnetes System) bleibt 3D deaktiviert.
        ...
        if enabled:
            ...
            self.statusBar().showMessage("3D-Ansicht aktiv — Links ziehen: Orbit, Rechts ziehen: Pan, Mausrad: Zoom, Ctrl+Mausrad: Höhe")
        else:
            self.center_stack.setCurrentWidget(self.view)
    ...
    def _on_3d_height_delta(self, obj, delta_world: float):
        if obj is None or isinstance(obj, ZoneItem):
            return
        pparts = [p.strip() for p in obj.data.get("pos", "0,0,0").split(",")]
        fx = float(pparts[0]) if len(pparts) > 0 and pparts[0] else 0.0
        fy = float(pparts[1]) if len(pparts) > 1 and pparts[1] else 0.0
        fz = float(pparts[2]) if len(pparts) > 2 and pparts[2] else (float(pparts[1]) if len(pparts) > 1 and pparts[1] else 0.0)
        fy += delta_world
        new_pos = f"{fx:.2f}, {fy:.2f}, {fz:.2f}"
        obj.data["_entries"] = [(k, new_pos if k.lower() == "pos" else v) for k, v in obj.data.get("_entries", [])]
    ...
    def _load_universe(self, game_path: str):
        ...
        # 3D in der Universumsansicht deaktivieren
        self.view3d_switch.blockSignals(True)
        self.view3d_switch.setChecked(False)
    ...
    def _load(self, path: str, restore: QTransform | None = None):
        ...
        # 3D-Schalter für normale Systemansicht wieder aktivieren
    ...
    def _jump_to_selected_from_combo(self):
        ...
        # center 3D view (works whether 3D tab is active or not)
        try:
