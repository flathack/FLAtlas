# TODO

## File Explorer
[X] gelöschte zeilen anzeigen / wiederherstellen
[X] undo soll auch funktionieren, auch wenn die datei geschlossen wurde und später wieder geöffnet wurde.
[X] time machine -> öffnet ein side to side view und mit einem schieberegler kann man alte versionen der Datei anzeigen lassen

## 2D Editor
[X] Ringe für jedes Objekt hinzufügen
[X] Move Objects button auch im Rechtsklick Menü eines Objektes anzeigen.
[X] mehrere inner system jump holes sollen möglich sein, bei ziel und quell system wird dann z. b. Li01a und Li01b genutzt. Beim nächsten dann Li01c und Li01d etc.
- Exclusion Zones: Exclusion Shells bei Add Exclusion Zones hinzufügen
- Zone Population: aktuell stürzt FL ab beim erstellung von Patrol Zones
### BASES
- Ship Dealer schiffe haben keine ingame Namen
- checken: mehrere Einträge, mal sehen obs auch bei neuen basen ist!
[Room_Info]
set_script = scripts\bases\li_09_Bar_hardpoint_R6.thn
scene = all, ambient, scripts\bases\li_09_bar_ambi_Li03_03.thn
scene = all, ambient, scripts\bases\li_09_bar_ambi_Li03_03.thn
scene = all, ambient, scripts\bases\li_09_bar_ambi_Li03_03.thn
scene = all, ambient, scripts\bases\li_09_bar_ambi_Li03_03.thn
scene = all, ambient, scripts\bases\li_09_bar_ambi_Li03_03.thn

## FL Atlas Settings
[X] button jetzt in Einstellungen im Tab Reset

## Faction Editor
- Neues Feature

## NPC Editor
- features komplettieren

## News Editor
- features komplettieren

## Rumor Editor
- features komplettieren

## Ship Editor 
- Einen Editor für FL Schiffe implementieren
- 